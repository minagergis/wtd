...
async function performComparison() {
    showProgress('Diffing in progress… hold on ⌛', 'Loading sheet data');
    ...
    try {
        ...
        const matchingBy = dateColumn ? `${keyColumn}+${dateColumn}` : keyColumn;

        // Show row count info
        const totalRows = Math.max(data1.length, data2.length);
        showProgress('Diffing in progress… hold on ⌛', `Processing ${totalRows.toLocaleString()} rows by ${matchingBy || 'position'}`);
...
function renderCombinedRowsChunked(tbody, matrix, columns, startRow, endRow) {
    ...
    function renderChunk() {
        ...
        for (let r = currentRow; r < chunkEnd; r++) {
            ...
            const rowChanges = matrix[r].map(cell => cell.type);
            const allUnchanged = rowChanges.every(type => type === 'unchanged');

            // Skip unchanged rows if diffs-only filter is active
            if (state.showDiffsOnly && allUnchanged) {
                continue;
            }
